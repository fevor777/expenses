  ...
  signInWithGoogle() {
    ...
    if (isAndroidWebView) {
      ...
      const tryLater = observer => {
        ...
        attempts++;
        if (attempts < max) setTimeout(() => tryLater(observer), interval);
        else {
          observer.next(null);
          observer.complete();
        }
    ...
    return from(this.afAuth.signInWithPopup(new GoogleAuthProvider())).pipe(
      catchError(err => {
        console.error('[AuthService] signInWithPopup error');
        return of(null);
      })
    );
  }
  ...
  private attachNativeTokenHandler() {
    ...
    if (!anyWindow.onNativeGoogleIdToken) {
      anyWindow.onNativeGoogleIdToken = (idToken: string) => {
        if (!idToken) {
          console.error('[AuthService] Empty idToken from native');
          return;
        }
        ...
        this.afAuth
          ...
          .catch(e =>
            console.error('[AuthService] signInWithCredential failed', e)
          );
      };
    }
  }
