...
export class PeriodSummaryService {
  ...
  getCurrentSummaries(): Observable<PeriodSummary[]> {
    ...
    const daysPassed =
      Math.floor(todayEnd.startOf('day').diff(monthStart, 'days').days) + 1; // inclusive days from 1..N
    const frameMonth = {
    ...
    const supersetStart = Math.min(
      frameWeek.start,
      frameMonth.start,
      frameLastMonth.start
    );
    const supersetFrame = { start: supersetStart, finish: frameMonth.finish };

    return combineLatest([
      this.fetch(supersetFrame),
      ...
      map(([res, categories]) => {
        ...
        const monthExpenses = all.filter(
          e => e.date >= frameMonth.start && e.date <= frameMonth.finish
        );
        const lastMonthExpenses = all.filter(
          e => e.date >= frameLastMonth.start && e.date <= frameLastMonth.finish
        );
        const weekExpenses = all.filter(
          e => e.date >= frameWeek.start && e.date <= frameWeek.finish
        );
        const todayExpenses = all.filter(
          e => e.date >= frameToday.start && e.date <= frameToday.finish
        );
        const yesterdayExpenses = all.filter(
          e => e.date >= frameYesterday.start && e.date <= frameYesterday.finish
        );

        const todaySnap = createPeriodSnapshot(
          frameToday,
          todayExpenses,
          categoryNames
        );
        const yesterdaySnap = createPeriodSnapshot(
          frameYesterday,
          yesterdayExpenses,
          categoryNames
        );
        const weekSnap = createPeriodSnapshot(
          frameWeek,
          weekExpenses,
          categoryNames
        );
        const monthSnap = createPeriodSnapshot(
          frameMonth,
          monthExpenses,
          categoryNames
        );
        const lastMonthSnap = createPeriodSnapshot(
          frameLastMonth,
          lastMonthExpenses,
          categoryNames
        );
        ...
        const summaries: PeriodSummary[] = [
          {
            key: 'today',
            title: todaySnap.frame.title,
            frame: toDateFrame(todaySnap.frame),
            paragraphs: todayNarr.paragraphs,
          },
          {
            key: 'yesterday',
            title: yesterdaySnap.frame.title,
            frame: toDateFrame(yesterdaySnap.frame),
            paragraphs: yesterdayNarr.paragraphs,
          },
          {
            key: 'week',
            title: weekSnap.frame.title,
            frame: toDateFrame(weekSnap.frame),
            paragraphs: weekNarr.paragraphs,
          },
          {
            key: 'month',
            title: monthSnap.frame.title,
            frame: toDateFrame(monthSnap.frame),
            paragraphs: monthNarr.paragraphs,
          },
          {
            key: 'lastMonth',
            title: lastMonthSnap.frame.title,
            frame: toDateFrame(lastMonthSnap.frame),
            paragraphs: lastMonthNarr.paragraphs,
          },
        ];
        return summaries;
      })
