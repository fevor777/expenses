...
export class CategoryAnalyticsService {
    buildAggregates(expenses: Expense[]): CategoryAggregate[] {
        ...
        return Object.values(map).map(a => ({ ...a, avg: a.count ? Math.round((a.total / a.count) * 100) / 100 : 0 }));
    }

    buildPareto(aggs: CategoryAggregate[]): { data: CategoryAggregate[]; cumulativePercents: number[] } {
        const sorted = [...(aggs || [])].sort((a, b) => b.total - a.total);
        const grand = sorted.reduce((s, a) => s + a.total, 0) || 1;
        let run = 0;
        const cum: number[] = [];
        sorted.forEach(a => { run += a.total; cum.push(Math.round((run / grand) * 10000) / 100); });
