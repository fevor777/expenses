...
export function registerUpdateBudgetTool(server, deps) {
    server.registerTool('update_budget', {
        description: 'Update the authenticated user\'s budget settings. Provide at least one of: value, period, periodStartTs, timezone, or minDayLimit.',
        inputSchema: updateBudgetSchema,
        outputSchema: updateBudgetResultSchema,
    }, async (input) => {
        return executeTool(deps, 'update_budget', async () => {
            assertAtLeastOneDefinedField(input, ['value', 'period', 'periodStartTs', 'timezone', 'minDayLimit'], 'At least one budget field must be provided');
