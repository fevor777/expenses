...
export function registerBudgetPeriodSummaryTool(server, deps) {
    server.registerTool('budget_period_summary', {
        description: 'Return an aggregated summary for one budget period selected by periodOffset. This tool is budget-based, not calendar-month-based. Use periodOffset 0 for the current period and -1 for the previous period. Remaining budget and limit summaries use only expenses where includeInBalance is true. Optional input: includeCategoryBreakdown.',
