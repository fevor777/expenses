import { budgetPeriodSummarySchema, budgetPeriodSummaryShape, } from '../domain/schemas.js';
...
export function registerBudgetPeriodSummaryTool(server, deps) {
    server.tool('budget_period_summary', 'Return a budget summary for a shifted budget period. Use periodOffset 0 for the current period, -1 for the previous period. Remaining budget uses only expenses where includeInBalance is true, and period boundaries are shifted copies of the configured budget start date and duration.', budgetPeriodSummaryShape, async (input) => {
