...
function resolveRelativeDateRange(input) {
    const relativeType = input.relativeType;
    const timezone = requireTimezone(input.timezone, 'relative');
    if (!relativeType) {
        throw new Error('Invalid resolve_date_range input: relativeType is required when pattern is relative');
    }
    ...
    return buildResolvedRange('relative', startLocal, endLocal, timezone, { relativeType });
}
function resolveCalendarRange(input) {
    ...
    if (!startDateLocal) {
        throw new Error('Invalid resolve_date_range input: startDateLocal is required when pattern is calendar_range');
    }
    if (!endDateLocal) {
        throw new Error('Invalid resolve_date_range input: endDateLocal is required when pattern is calendar_range');
    }
    const startLocal = parseLocalDate(startDateLocal, 'startDateLocal');
    const endLocal = parseLocalDate(endDateLocal, 'endDateLocal');
    return buildResolvedRange('calendar_range', startLocal, endLocal, timezone, { startDateLocal, endDateLocal });
}
function resolveAbsoluteRange(input) {
    const startDate = input.startDate;
    const endDate = input.endDate;
    const timezone = input.timezone?.trim() || undefined;
    if (startDate === undefined) {
        throw new Error('Invalid resolve_date_range input: startDate is required when pattern is absolute');
    }
    if (endDate === undefined) {
        throw new Error('Invalid resolve_date_range input: endDate is required when pattern is absolute');
    }
    if (startDate > endDate) {
        throw new Error('Invalid resolve_date_range input: startDate must be less than or equal to endDate');
    }
    if (timezone) {
    ...
    return {
        ...
        rangeDisplay: buildRangeDisplay(formatLocalDate(startParts), formatLocalDate(endParts), timezone),
        patternMetadata: {
            mode: 'absolute',
        },
    };
}
function buildResolvedRange(pattern, startLocal, endLocal, timezone, patternMetadata) {
    assertValidTimeZone(timezone);
...
function requireTimezone(timezone, pattern) {
    const value = timezone?.trim();
    if (!value) {
        throw new Error(`Invalid resolve_date_range input: timezone is required when pattern is ${pattern}`);
    }
...
function parseLocalDate(value, field) {
    ...
    if (!match) {
        throw new Error(`Invalid resolve_date_range input: ${field} must use YYYY-MM-DD`);
    }
    ...
    if (date.getUTCFullYear() !== year ||
        ...
        date.getUTCDate() !== day) {
        throw new Error(`Invalid resolve_date_range input: ${field} must be a real calendar date`);
    }
...
function zonedDateTimeToUtcMs(date, timezone, boundary) {
    const desiredHour = boundary === 'start' ? 0 : 23;
    const desiredMinute = boundary === 'start' ? 0 : 59;
    const desiredSecond = boundary === 'start' ? 0 : 59;
    const desiredAsUtc = Date.UTC(date.year, date.month - 1, date.day, desiredHour, desiredMinute, desiredSecond, 0);
    ...
    if (resolved.year !== date.year ||
        ...
        throw new Error('Invalid resolve_date_range input: could not resolve date in the requested timezone');
    }
    return boundary === 'end' ? candidate + 999 : candidate;
}
...
function diffLocalDays(start, end) {
...
function buildRangeDisplay(startDateLocal, endDateLocal, timezone) {
    const base = startDateLocal === endDateLocal
        ? startDateLocal
