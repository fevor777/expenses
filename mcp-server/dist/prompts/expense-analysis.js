export function registerExpenseAnalysisPrompts(server) {
    registerExpenseAnalysisPrompt(server, 'analyze_expenses_today', 'Анализ расходов за сегодня со сравнением со вчера.', 'today');
    registerExpenseAnalysisPrompt(server, 'analyze_expenses_yesterday', 'Анализ расходов за вчера со сравнением с позавчера.', 'yesterday');
    registerExpenseAnalysisPrompt(server, 'analyze_expenses_week', 'Анализ расходов за текущую неделю со сравнением с прошлой неделей за тот же прогресс.', 'week');
    registerExpenseAnalysisPrompt(server, 'analyze_expenses_month', 'Анализ расходов за текущий месяц со сравнением с прошлым месяцем за тот же прогресс.', 'month');
}
function registerExpenseAnalysisPrompt(server, name, description, period) {
...
function buildAnalysisPrompt(frames) {
...
function buildAnalysisFrames(period, now = new Date()) {
    switch (period) {
        case 'today': {
        ...
        case 'yesterday': {
            ...
            return {
                ...
                previous: {
                    ...
                    displayDate: formatRange(startOfDay(dayBeforeYesterday), endOfDay(dayBeforeYesterday)),
                },
            };
        }
        case 'week': {
        ...
        case 'month': {
            ...
            const previousFinish = previousFinishCandidate.getTime() > previousMonthEnd.getTime()
                ? previousMonthEnd
