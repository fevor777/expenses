...
import { OAuthClientsRepository, } from './firestore/oauth-clients.repository.js';
...
export function registerOAuthBrokerRoutes(app, config, logger) {
    if (!config.oauth || !config.oauthBroker || config.oauthBroker.mode !== 'google') {
        return;
    }
    const broker = createGoogleBroker(config.oauth, config.oauthBroker, new OAuthClientsRepository(getFirestoreClient(config)), logger);
...
function createGoogleBroker(oauthConfig, brokerConfig, oauthClientsRepository, logger) {
    ...
    return {
        ...
        async handleRegister(req, res) {
            ...
            const grantTypes = readClientMetadata(() => readOptionalStringArrayBodyParam(req, 'grant_types') ?? ['authorization_code']);
            const responseTypes = readClientMetadata(() => readOptionalStringArrayBodyParam(req, 'response_types') ?? ['code']);
            ...
            const clientSecret = tokenEndpointAuthMethod === 'none' ? undefined : randomBytes(24).toString('hex');
            const registration = {
                clientId,
        ...
        async handleAuthorize(req, res) {
            ...
            const client = await requireRegisteredClient(brokerConfig, oauthClientsRepository, clientId);
            ...
            const brokerState = await signBrokerState({
                ...
                nonce,
            }, oauthConfig, brokerConfig, keyPromise);
        ...
        async handleCallback(req, res) {
            ...
            const downstreamState = await verifyBrokerState(brokerStateToken, oauthConfig, brokerConfig, keyPromise);
            const identity = await exchangeGoogleCode(code, callbackUrl, brokerConfig, googleJwksPromise, logger);
        ...
        async handleToken(req, res) {
            ...
            const client = await requireRegisteredClient(brokerConfig, oauthClientsRepository, clientAuthentication.clientId);
            ...
            if (grantType === 'refresh_token') {
                ...
                const nextRefreshToken = await signRefreshToken(refreshTokenPayload, oauthConfig, brokerConfig, keyPromise);
                res.status(200).json({
            ...
            const accessToken = await signAccessToken(authorizationCode, oauthConfig, brokerConfig, keyPromise);
...
async function loadSigningKeys(brokerConfig) {
...
async function signBrokerState(state, oauthConfig, brokerConfig, keyPromise) {
...
async function verifyBrokerState(token, oauthConfig, brokerConfig, keyPromise) {
    const keys = await keyPromise;
...
async function signAuthorizationCode(payload, oauthConfig, brokerConfig, keyPromise) {
    const keys = await keyPromise;
...
async function verifyAuthorizationCode(token, oauthConfig, keyPromise) {
    const keys = await keyPromise;
    const { payload } = await jwtVerify(token, keys.verificationKey, {
        issuer: oauthConfig.issuer,
        audience: oauthConfig.issuer,
...
async function signAccessToken(payload, oauthConfig, brokerConfig, keyPromise) {
    const keys = await keyPromise;
...
async function signRefreshToken(payload, oauthConfig, brokerConfig, keyPromise) {
    const keys = await keyPromise;
    return new SignJWT({
        scope: payload.scope,
        email: payload.email,
...
async function verifyRefreshToken(token, oauthConfig, keyPromise) {
    const keys = await keyPromise;
    const { payload } = await jwtVerify(token, keys.verificationKey, {
        issuer: oauthConfig.issuer,
        audience: oauthConfig.issuer,
...
async function exchangeGoogleCode(code, redirectUri, brokerConfig, googleJwksPromise, logger) {
...
function enforceAllowedIdentity(oauthConfig, brokerConfig, identity) {
...
async function requireRegisteredClient(brokerConfig, oauthClientsRepository, clientId) {
...
function validateClientRequest(client, redirectUri) {
    if (!client.redirectUris.includes(redirectUri)) {
        throw new Error('redirect_uri is not allowed');
    }
}
function readTokenClientAuthentication(req) {
...
function isClientAuthenticationValid(client, authentication) {
...
function readTokenEndpointAuthMethod(req) {
...
function validateRegistrationGrantTypes(grantTypes, responseTypes) {
    ...
    if (!uniqueGrantTypes.has('authorization_code')) {
        throw new InvalidClientMetadataError('authorization_code grant type is required');
    }
    for (const grantType of uniqueGrantTypes) {
        if (!supportedGrantTypes.has(grantType)) {
            throw new InvalidClientMetadataError('Only authorization_code and refresh_token grant types are supported');
        }
    }
...
function validateRefreshTokenSupport(grantTypes, tokenEndpointAuthMethod) {
    if (tokenEndpointAuthMethod === 'none' && !grantTypes.includes('authorization_code')) {
        throw new InvalidClientMetadataError('public clients must support authorization_code when requesting tokens');
    }
}
function readClientMetadata(reader) {
    try {
        return reader();
    }
    catch (error) {
...
function validateRedirectUris(redirectUris, allowedRedirectSchemes) {
    for (const redirectUri of redirectUris) {
        validateRedirectUri(redirectUri, allowedRedirectSchemes);
    }
}
function validateRedirectUri(redirectUri, allowedRedirectSchemes) {
    ...
    throw new Error('redirect_uri must use https, localhost http, or an explicitly allowed application-specific scheme');
}
...
function readSingleQueryParam(req, name) {
...
function readOptionalQueryParam(req, name) {
    const value = req.query[name];
    return typeof value === 'string' && value.trim().length > 0 ? value : undefined;
}
function readBodyParam(req, name) {
...
function readOptionalBodyString(req, name) {
    const value = req.body?.[name];
    return typeof value === 'string' && value.trim().length > 0 ? value : undefined;
}
function readOptionalStringArrayBodyParam(req, name) {
    const value = req.body?.[name];
...
function readStringArrayBodyParam(req, name) {
