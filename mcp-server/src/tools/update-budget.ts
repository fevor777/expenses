...
export function registerUpdateBudgetTool(
  ...
  server.registerTool(
    ...
    {
      description:
        'Update the authenticated user\'s budget settings. Provide at least one of: value, period, periodStartTs, timezone, minDayLimit, or limits.',
    ...
    async input => {
      return executeTool(deps, 'update_budget', async () => {
        assertAtLeastOneDefinedField(
          input,
          ['value', 'period', 'periodStartTs', 'timezone', 'minDayLimit', 'limits'],
